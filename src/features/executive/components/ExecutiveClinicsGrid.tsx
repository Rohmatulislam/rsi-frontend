...
export const ExecutiveClinicsGrid = ({ items }: ExecutiveClinicsGridProps) => {
    return (
            ...
            <div className="container mx-auto px-4">
                <div className="flex flex-col md:flex-row items-end justify-between mb-16 gap-6">
                    <div className="max-w-2xl">
                        ...
                        <h2 className="text-3xl md:text-4xl font-bold mb-4">Poliklinik Executive</h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {items.filter(item => item.isActive).map((item, idx) => {
                        const baseSpecialty = item.name.replace(/ eksekutif/gi, '');
                        return (
                            <div key={item.id} className="bg-card border border-border/40 rounded-[2rem] p-8 shadow-sm hover:shadow-2xl hover:border-amber-500/20 transition-all group relative overflow-hidden h-full flex flex-col">
                                ...
                                <h3 className="text-xl font-bold mb-3 group-hover:text-amber-600 transition-colors">{item.name}</h3>
                                <p className="text-sm text-muted-foreground mb-8 flex-grow leading-relaxed">
                                    {item.description || `Layanan unggulan spesialis ${item.name} dengan waktu konsultasi yang lebih luas dan pendampingan personal.`}
                                </p>
